    ...
    return new Promise(async (resolve, reject) => {
      if (originalBook && newBook) {
        ...
        if (isMatch) {
          ...
          resolve(
            `'${originalBook}' book name is replace with '${newBook}' book name`
          );
        } else {
          reject(
            `'${originalBook}' book name is not at all present inside the book list`
          );
        }
      } else {
  ...
  try {
    return new Promise(async (resolve, reject) => {
      ...
      if (isMatch) {
        ...
        reject(
          `'${book}' book is not at all present in an book list to delete`
        );
      }
    });
  } catch (err) {
